...
import { LoginFormViewModelReactiveLazy } from './login-form.viewmodel';
import { LoginFormPresenterReactive } from './login-form.presenter';
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const loginFormVM = new LoginFormViewModelReactiveLazy();
    const loginFormPresenter = new LoginFormPresenterReactive(loginFormVM);
    ...
    // validate password
    watch(loginFormVM.password, (passwd) => {
      loginInteractor.validatePassword(passwd);
    });

    // onClick login button
    const onLogInClick = async () => {
      await loginInteractor.login(loginFormVM.username.value, loginFormVM.password.value);
    };
